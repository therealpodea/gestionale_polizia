...
if DATABASE_URL:
    ...
    def _fix(q): return q.replace("?", "%s").replace("INSERT OR IGNORE","INSERT").replace("INSERT OR REPLACE","INSERT")

    def init_db():
        conn = get_db(); c = conn.cursor()
        c.execute('''CREATE TABLE IF NOT EXISTS agents (id TEXT PRIMARY KEY, nome TEXT NOT NULL, cognome TEXT NOT NULL, cf TEXT NOT NULL, discord TEXT NOT NULL, grado TEXT NOT NULL, stato TEXT NOT NULL DEFAULT 'Attivo', sanzione TEXT, note TEXT, "dataIngresso" TEXT NOT NULL)''')
        ...
        defaults = {"pwd":"estovia2026","gradi":json.dumps(["Allievo Poliziotto","Agente","Agente Scelto","Assistente","Assistente Coordinatore","Assistente Capo","Vice Sovrintendente","Sovrintendente","Sovrintendente Capo","Sovrintendente Superiore","Vice Ispettore","Ispettore","Ispettore Capo","Ispettore Superiore","Sostituto Commissario","Vice Commissario","Commissario","Commissario Capo","Primo Dirigente","Dirigente Aggiunto","Dirigente Superiore","Dirigente Generale"]),"sanzioni":json.dumps(["AVVISO FORMALE 1","AVVISO FORMALE 2","RICHIAMO 1","RICHIAMO 2","RICHIAMO 3","SOSPENSIONE"]),"logo":""}
        for k,v in defaults.items():
            c.execute("INSERT INTO settings (key,value) VALUES (%s,%s) ON CONFLICT (key) DO NOTHING",(k,v))
        conn.commit(); conn.close()

    def db_fetchall(q, p=()):
        conn=get_db(); cur=conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor); cur.execute(_fix(q),p); rows=cur.fetchall(); conn.close(); return [dict(r) for r in rows]
    def db_fetchone(q, p=()):
        conn=get_db(); cur=conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor); cur.execute(_fix(q),p); row=cur.fetchone(); conn.close(); return dict(row) if row else None
    def db_execute(q, p=()):
        conn=get_db(); cur=conn.cursor(); cur.execute(_fix(q),p); conn.commit(); conn.close()
    def db_execute_many(qps):
        conn=get_db(); cur=conn.cursor()
        for q,p in qps: cur.execute(_fix(q),p)
        conn.commit(); conn.close()
    def upsert_setting(key, val):
        conn=get_db(); cur=conn.cursor(); cur.execute("INSERT INTO settings (key,value) VALUES (%s,%s) ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value",(key,val)); conn.commit(); conn.close()
    ...
    def init_db():
        ...
        conn.commit(); conn.close()
    def db_fetchall(q,p=()):
        conn=get_db(); rows=conn.execute(q,p).fetchall(); conn.close(); return [dict(r) for r in rows]
    def db_fetchone(q,p=()):
        conn=get_db(); row=conn.execute(q,p).fetchone(); conn.close(); return dict(row) if row else None
    def db_execute(q,p=()):
...
def norm(row, key): return row.get(key) or row.get(key.lower()) or row.get(key.upper()) or ""
...
def update_settings(s: SettingsUpdate):
    if s.pwd: upsert_setting("pwd",s.pwd)
    if s.gradi is not None: upsert_setting("gradi",json.dumps(s.gradi))
    if s.sanzioni is not None: upsert_setting("sanzioni",json.dumps(s.sanzioni))
    if s.logo is not None: upsert_setting("logo",s.logo)
    return {"ok":True}
