...
# ════════════════════════════════════════════════════════════
#  PATCH: aggiunta colonna agent_pwd alla tabella agents
# ════════════════════════════════════════════════════════════
def migrate_db():
    try:
        if DATABASE_URL:
            conn = get_db()
            c = conn.cursor()
            c.execute("ALTER TABLE agents ADD COLUMN agent_pwd TEXT")
            conn.commit()
            conn.close()
        else:
            conn = get_db()
            conn.execute("ALTER TABLE agents ADD COLUMN agent_pwd TEXT")
            conn.commit()
            conn.close()
    except Exception:
...
# ══ NUOVE ROUTE PASSWORD AGENTI ══

@app.post("/api/login/agente")
def login_agente_v2(body: dict):
    ...
    if row.get("stato") == "Congedato":
        raise HTTPException(status_code=403, detail="Account congedato")
    agent_pwd = row.get("agent_pwd") or row.get("agentpwd") or row.get("agent_pwd".lower())
    if not agent_pwd:
...
def get_agenti_passwords():
    ...
    return [{"id": r["id"], "nome": r["nome"], "cognome": r["cognome"], "discord": r["discord"],
             "has_password": bool(r.get("agent_pwd"))} for r in rows]
