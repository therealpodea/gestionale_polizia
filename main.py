...
if DATABASE_URL:
    ...
    def _q(s): return s.replace("?","%s").replace("INSERT OR IGNORE INTO","INSERT INTO").replace("INSERT OR REPLACE INTO","INSERT INTO").replace("INSERT OR IGNORE","INSERT").replace("INSERT OR REPLACE","INSERT")
    ...
    def init_db():
        ...
        # Documenti di default (solo se tabella vuota)
        count = c.execute("SELECT COUNT(*) FROM documenti") if False else None
        cur2 = conn.cursor(); cur2.execute("SELECT COUNT(*) FROM documenti"); cnt = cur2.fetchone()[0]
        if cnt == 0:
            docs_default=[
                ...
                (str(uuid.uuid4())[:12],"Normativa Cautelare","Normativa di garanzia e responsabilità","https://docs.google.com/document/d/1u4OIkEOxXm5zhs0e_bd4nUJPHBz4rOFeF0z4910LbRM/edit","🛡️","","normative",1),
                (str(uuid.uuid4())[:12],"Costituzione dello Stato d'Estovia","Normativa fondamentale dello Stato","https://docs.google.com/document/d/1AxN9YP6YTot_vNNpKkZRzVE6hUvGQ0ertdrGt2T85eg/edit","🏛️","","fondamentale",1),
                (str(uuid.uuid4())[:12],"Codice Penale","Codice penale dello Stato d'Estovia","https://docs.google.com/document/d/1qcHd2Y3LMf_6VTIOuh2St-LVyocK6xv_aPT_Q841PcU/edit","⚖️","","codici",3),
            ...
            for d in docs_default:
                c.execute("INSERT INTO documenti (id,titolo,descrizione,url,icona,stato,categoria,ordine) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",d)
        conn.commit(); conn.close()
    ...
    def init_db():
        ...
        cnt = c.execute("SELECT COUNT(*) FROM documenti").fetchone()[0]
        if cnt == 0:
            docs=[
                (str(uuid.uuid4())[:12],"Regolamento Interno","Regolamento interno della Polizia d'Estovia","https://docs.google.com/document/d/1YRyLrOUOelm3PZMIvAZ4NOmLt92qQhVfB55ueU0s3Oo/edit","📋","","regolamenti",1),
                (str(uuid.uuid4())[:12],"Protocolli Operativi Interni","Protocolli e procedure operative","https://docs.google.com/document/d/1tBs-bCDnqOwWT3eRDaoZvsqVidhyAKM4bHhRSz24SkI/edit","⚙️","","regolamenti",2),
            ...
            for d in docs:
                c.execute("INSERT INTO documenti (id,titolo,descrizione,url,icona,stato,categoria,ordine) VALUES (?,?,?,?,?,?,?,?)",d)
        conn.commit(); conn.close()
