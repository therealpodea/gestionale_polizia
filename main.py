...
if DATABASE_URL:
    ...
    def init_db():
        ...
        docs_default=[
            ...
            (str(uuid.uuid4())[:12],"Codice Civile","","","📖","In fase di emissione (Admin TheRealPodea)","codici",1),
            (str(uuid.uuid4())[:12],"Codice Stradale","","","🚗","In fase di emissione (Admin TheRealPodea)","codici",2),
        ]
        for d in docs_default:
            c.execute("INSERT OR IGNORE INTO documenti (id,titolo,descrizione,url,icona,stato,categoria,ordine) VALUES (?,?,?,?,?,?,?,?)",d)
        conn.commit(); conn.close()
...
def get_pec():
    ...
    for r in rows:
        r["letta"]=bool(r.get("letta",False))
    return rows
...
def leggi_pec(pec_id: str):
    db_execute("UPDATE pec SET letta=1 WHERE id=?",(pec_id,))
    return {"ok":True}

@app.delete("/api/pec/{pec_id}")
def delete_pec(pec_id: str):
    db_execute("DELETE FROM pec WHERE id=?",(pec_id,))
    return {"ok":True}
...
@app.post("/api/documenti", dependencies=[Depends(check_auth)])
def create_documento(doc: Documento):
...
def delete_documento(doc_id: str):
    db_execute("DELETE FROM documenti WHERE id=?",(doc_id,))
    return {"ok":True}
...
def update_segnalazione(seg_id: str, body: StatoUpdate):
    db_execute("UPDATE segnalazioni SET stato=? WHERE id=?",(body.stato,seg_id))
    return {"ok":True}

@app.delete("/api/segnalazioni/{seg_id}", dependencies=[Depends(check_auth)])
def delete_segnalazione(seg_id: str):
    db_execute("DELETE FROM segnalazioni WHERE id=?",(seg_id,))
    return {"ok":True}
