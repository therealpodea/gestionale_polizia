...
async def delete_azione(aid: int, _=Depends(verify_api_key)):
    if USE_POSTGRES:
        pool = await get_pg_pool(); async with pool.acquire() as conn: await conn.execute("DELETE FROM azioni WHERE id=$1", aid)
    else:
        db = _sqlite_conn(); db.execute("DELETE FROM azioni WHERE id=?", (aid,)); db.commit(); db.close()
    return {"ok": True}
...
async def delete_comunicato(cid: int, _=Depends(verify_api_key)):
    if USE_POSTGRES:
        pool = await get_pg_pool(); async with pool.acquire() as conn: await conn.execute("DELETE FROM comunicati WHERE id=$1", cid)
    else:
        db = _sqlite_conn(); db.execute("DELETE FROM comunicati WHERE id=?", (cid,)); db.commit(); db.close()
    return {"ok": True}
...
async def mark_pec_letta(pid: int, _=Depends(verify_api_key)):
    if USE_POSTGRES:
        pool = await get_pg_pool(); async with pool.acquire() as conn: await conn.execute("UPDATE pec SET letta=1 WHERE id=$1", pid)
    else:
        db = _sqlite_conn(); db.execute("UPDATE pec SET letta=1 WHERE id=?", (pid,)); db.commit(); db.close()
    return {"ok": True}

@app.delete("/api/pec/{pid}")
async def delete_pec(pid: int, _=Depends(verify_api_key)):
    if USE_POSTGRES:
        pool = await get_pg_pool(); async with pool.acquire() as conn: await conn.execute("DELETE FROM pec WHERE id=$1", pid)
    else:
        db = _sqlite_conn(); db.execute("DELETE FROM pec WHERE id=?", (pid,)); db.commit(); db.close()
    return {"ok": True}
...
async def update_stato_segnalazione(sid: int, request: Request, _=Depends(verify_api_key)):
    body = await request.json(); stato = body.get("stato", "")
    if USE_POSTGRES:
        pool = await get_pg_pool(); async with pool.acquire() as conn: await conn.execute("UPDATE segnalazioni SET stato=$1 WHERE id=$2", stato, sid)
    else:
        db = _sqlite_conn(); db.execute("UPDATE segnalazioni SET stato=? WHERE id=?", (stato,sid)); db.commit(); db.close()
    return {"ok": True}

@app.delete("/api/segnalazioni/{sid}")
async def delete_segnalazione(sid: int, _=Depends(verify_api_key)):
    if USE_POSTGRES:
        pool = await get_pg_pool(); async with pool.acquire() as conn: await conn.execute("DELETE FROM segnalazioni WHERE id=$1", sid)
    else:
        db = _sqlite_conn(); db.execute("DELETE FROM segnalazioni WHERE id=?", (sid,)); db.commit(); db.close()
    return {"ok": True}
...
async def delete_documento(did: int, _=Depends(verify_api_key)):
    if USE_POSTGRES:
        pool = await get_pg_pool(); async with pool.acquire() as conn: await conn.execute("DELETE FROM documenti WHERE id=$1", did)
    else:
        db = _sqlite_conn(); db.execute("DELETE FROM documenti WHERE id=?", (did,)); db.commit(); db.close()
    return {"ok": True}
