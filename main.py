...
if DATABASE_URL:
    ...
    def init_db():
        ...
        conn.commit()
        # Rinomina colonne quoted → unquoted (migrazione tabelle esistenti)
        renames = [
            ('agents', '"dataIngresso"', 'dataIngresso'),
            ('history', '"agentId"', 'agentId'),
            ('history', '"agentNome"', 'agentNome'),
            ('comunicati', '"readBy"', 'readBy'),
            ('pec', '"mittenteId"', 'mittenteId'),
            ('pec', '"mittenteNome"', 'mittenteNome'),
            ('pec', '"destinatarioId"', 'destinatarioId'),
            ('pec', '"destinatarioNome"', 'destinatarioNome'),
            ('segnalazioni', '"mittenteId"', 'mittenteId'),
            ('segnalazioni', '"mittenteNome"', 'mittenteNome'),
        ]
        for tbl, old_col, new_col in renames:
            try:
                c.execute(f"ALTER TABLE {tbl} RENAME COLUMN {old_col} TO {new_col}")
                conn.commit()
...
def create_documento(doc: Documento, x_api_key: str = Header(None), x_session_token: str = Header(None)):
    ...
    if x_api_key == API_KEY:
        ...
        if not sess:
            raise HTTPException(status_code=401, detail="Sessione non valida")
        if doc.categoria not in ("altro", "rapporti"):
