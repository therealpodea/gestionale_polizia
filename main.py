...
# Bot token per recuperare i nomi dei ruoli (opzionale ma raccomandato)
...
# Discord IDs degli utenti che devono SEMPRE essere riconosciuti come dirigenza (separati da virgola)
ADMIN_DISCORD_IDS = set(os.getenv("ADMIN_DISCORD_IDS", "").split(",")) if os.getenv("ADMIN_DISCORD_IDS") else set()

USE_POSTGRES = bool(DATABASE_URL)

ROLE_DIRIGENZA = {
    "staff","dirigenza","dirigente generale","direttore generale","ispettorato","sovrintendenza",
    "direttore","vice direttore","commissario","ispettore capo","ispettore",
    "sovrintendente capo","sovrintendente","assistente capo","admin","amministratore"
}
...
def root():
    import os
    html_path = os.path.join(os.path.dirname(__file__), "gestionale.html")
    if not os.path.exists(html_path):
        # Fallback: cerca nella directory di lavoro corrente
        html_path = "gestionale.html"
...
async def discord_callback(code: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        # PRIORITY 1: Check if Discord ID is in admin override list
        if ADMIN_DISCORD_IDS and discord_id in ADMIN_DISCORD_IDS:
            access_level = "dirigenza"
            role_names = ["admin_override"]
        
        # PRIORITY 2: Check guild membership and roles
        elif DISCORD_GUILD:
            ...
            if member_resp.status_code == 200:
                ...
                username = member_data.get("nick", username) or username
                
                # Try to get role names using bot token
                if DISCORD_BOT_TOKEN and role_ids:
                    try:
                        guild_roles_resp = await client.get(
                            f"https://discord.com/api/guilds/{DISCORD_GUILD}/roles",
                            headers={"Authorization": f"Bot {DISCORD_BOT_TOKEN}"})
                        
                        if guild_roles_resp.status_code == 200:
                            guild_roles = guild_roles_resp.json()
                            role_map = {r["id"]: r["name"].lower() for r in guild_roles}
                            ...
                            for role_id in role_ids:
                                if role_id in role_map:
                                    ...
                                    # Check if this role matches dirigenza keywords
                                    for keyword in ROLE_DIRIGENZA:
                                        if keyword in role_name:
                                            access_level = "dirigenza"
                                            break
                                    
                                    if access_level == "dirigenza":
                                        break
                    except Exception as e:
                        print(f"Error getting guild roles: {e}")
                
                # Fallback: check nickname/username for dirigenza keywords
                if access_level == "agente":
                    user_lower = username.lower()
                    for keyword in ROLE_DIRIGENZA:
                        if keyword in user_lower:
                            access_level = "dirigenza"
                            role_names.append(f"nickname:{keyword}")
                            break
    ...
    if USE_POSTGRES:
        ...
        async with pool.acquire() as conn:
            await conn.execute("""INSERT INTO sessions
                ...
                token, discord_id, username, avatar, json.dumps(role_names), access_level, expires)
    else:
        db = _sqlite_conn()
...
# ─── ALIAS ROUTES (compatibilità frontend) ────────────────────────────────────
@app.get("/api/agents")
async def list_agents_alias(_=Depends(verify_api_key)):
    return await list_agenti(_)

@app.get("/api/agents/{aid}")
async def get_agent_alias(aid: int, _=Depends(verify_api_key)):
    return await get_agente(aid, _)

@app.post("/api/agents")
async def add_agent_alias(body: AgentePUT, _=Depends(verify_api_key)):
    return await add_agente(body, _)

@app.put("/api/agents/{aid}")
async def update_agent_alias(aid: int, body: AgentePUT, _=Depends(verify_api_key)):
    return await update_agente(aid, body, _)

@app.delete("/api/agents/{aid}")
async def delete_agent_alias(aid: int, _=Depends(verify_api_key)):
    return await delete_agente(aid, _)

@app.get("/api/history")
async def list_history_alias(_=Depends(verify_api_key)):
    return await list_azioni(_)

@app.post("/api/history")
async def add_history_alias(body: AzionePOST, _=Depends(verify_api_key)):
    return await add_azione(body, _)

@app.post("/api/comunicati/{cid}/read")
async def mark_read_alias(cid: int, request: Request, _=Depends(verify_api_key)):
    return await mark_letto(cid, request, _)

@app.put("/api/pec/{pid}/leggi")
async def mark_pec_leggi_alias(pid: int, _=Depends(verify_api_key)):
    return await mark_pec_letta(pid, _)
